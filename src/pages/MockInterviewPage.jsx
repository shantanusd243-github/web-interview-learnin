    ...
    <div id="page-mock" className="page active">
      ...
      <div className="mock-card">
        <div style={{ fontSize: 12, color: '#64748b', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
          Question {pos + 1 || (currentQ ? 1 : 0)} {history.length > 0 && <>of {history.length}</>}
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginTop: 8 }}>
          {!currentQ && (
            <button className="mock-answer-btn" onClick={fetchNext} style={{ width: '100%', maxWidth: 260 }}>
          ...
          {currentQ && (
            <>
              ...
              <button className="mock-answer-btn" style={{ background: '#1e293b' }} onClick={fetchNext}>
                Next →
              </button>
            </>
          )}
          ...
          <div className="mock-nav">
            <button
              ...
              style={{
                ...
                opacity: pos <= 0 ? 0.5 : 1, minHeight: 40,
              }}
            >
              ← Prev
            </button>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
              <button
                ...
                style={{ background: '#064e3b', color: '#34d399', border: 'none', padding: '7px 16px', borderRadius: 6, cursor: 'pointer', fontSize: 13, minHeight: 40 }}
              >✓ Got it</button>
              <button
                onClick={() => handleMark('weak')}
                style={{ background: '#7f1d1d', color: '#f87171', border: 'none', padding: '7px 16px', borderRadius: 6, cursor: 'pointer', fontSize: 13, minHeight: 40 }}
              >✗ Weak</button>
            </div>
